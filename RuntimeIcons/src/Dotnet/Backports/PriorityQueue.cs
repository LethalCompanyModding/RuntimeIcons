    ...
    public class PriorityQueue<TElement, TPriority>
    {
        ...
        public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
        {
            ...
            void ICollection.CopyTo(Array array, int index)
            {
                ...
                if (array.Rank != 1)
                {
                    throw new ArgumentException("Multidimensional Arrays not supported", nameof(array));
                }

                if (array.GetLowerBound(0) != 0)
                {
                    throw new ArgumentException("Lower bound must be non-zero", nameof(array));
                }

                if (index < 0 || index > array.Length)
                {
                    throw new ArgumentOutOfRangeException(nameof(index), index, "must be less or equal array lenght");
                }
                ...
                {
                    throw new ArgumentException("Incompativle array type", nameof(array));
                }
            }
